...
/**
 * Class with controller methods that are shared among controllers to reduce code duplication
 * 
 * @author Jack Croft
 *
 */
public class MultiPageController {
    ...
    /**
     * Determines whether or not an event was a valid action, all events are valid, except for key
     * presses, where only 'Enter' is acceptable for the key
     * 
     * @param e
     *            The event that is being determined valid or not
     * @return Whether or not the event is valid to continue
     */
    public static boolean isValidAction(Event event) {
    ...
    public static void loadView(Node node, String viewName) {
        try {
            Parent newScene = new FXMLLoader().load(ClassLoader.getSystemResourceAsStream("view/"
                    + viewName + ".fxml"));
