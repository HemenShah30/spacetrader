...
public class TradeEngine {
    ...
    public List<String> buy(GoodType tradeGood, int quantity, Marketplace market) {
        ...
        if (errors.isEmpty()) {
            ...
            market.setQuantity(tradeGood, market.getQuantity(tradeGood) - quantity);
        }
        return errors;
    }
     ...
     */
    public List<String> sell(GoodType tradeGood, int quantity, Marketplace market) {
     ...
     * @return The errors from the sell, if any
     */
    private List<String> validateSell(GoodType tradeGood, int quantity, Marketplace market) {
        List<String> errors = new ArrayList<String>();
    ...
    /**
     * Returns the maximum amount of a good a player can buy from the marketplace
     * 
     * @param good
    ...
    public int getMaximumBuyGoodAmount(GoodType good) {
        ...
        return Math.min(
                Math.min(ship.getCargoSize() - ship.getCurrCargo(), market.getQuantity(good)),
                ((int) player.getCredits() / (int) market.getBuyPrice(good, player)));
    }

    /**
     * Returns the maximum amount of a good that a player can sell to a marketplace
     * 
     * @param good
     *            The good being sold
    ...
    /**
     * Returns the maximum amount of good the given trader can buy or sell with the player
     * 
     * @param trader
    ...
    public int getMaximumTraderTradeAmount(Trader trader) {
        if (trader.isBuying()) {
            return Math.min(ship.amountInCargo(trader.getGoodOfInterest()), trader.getQuantity());
        } else {
            return Math.min(
                    Math.min(ship.getCargoSize() - ship.getCurrCargo(), trader.getQuantity()),
                    (int) (player.getCredits() / trader.getPrice()));
        }
    }

    /**
     * Executes a trade with a trader if possible, or returns a list of error strings
     * 
     * @param trader
    ...
    public List<String> tradeWithTrader(Trader trader, int quantity) {
        ...
        if (quantity > trader.getQuantity()) {
            errors.add("Trader will not " + (trader.isBuying() ? "buy" : "sell") + " this much "
                    + trader.getGoodOfInterest());
        }
        if (trader.isBuying()) {
            if (ship.amountInCargo(trader.getGoodOfInterest()) < quantity) {
                errors.add("You do not have " + quantity + " " + trader.getGoodOfInterest()
                        + " to sell");
            }
        } else {
    ...
    /**
     * Returns the list of errors generated by attempting a ShipUpgrade buy transaction
     * 
     * @param cost
    ...
    /**
     * Attempts a ShipUpgrade sell transaction between the player and the shipyard
     * 
     * @param upgrade
